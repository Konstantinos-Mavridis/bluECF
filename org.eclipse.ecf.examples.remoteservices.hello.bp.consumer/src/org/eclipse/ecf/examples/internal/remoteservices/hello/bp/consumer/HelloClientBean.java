...
public class HelloClientBean {
    ...
    /**
     * Binding method to be used with a referenced-interface implementation proxy injected by the activation framework
     * (e.g. Gemini Blueprint).
     * 
     * @param svcproxy
     */
    ...
    /** 
     * Because blueprint wraps the remote proxy based on the specific interface and IRemote... is not in the OBJECTCLASS i.e. can't use a filter to look up based on & (and) condition,
     * Inject ServiceReference, get native reference from it, and use that to look up service.  
     * 
     * I know this is not optimal and will provide a blueprint extension point to allow plugable proxy creation mechanism. I think there is a merit in that.
     * 
     * @param ref
     */
    public void bindHelloReference(ServiceReference ref) {
        // Print out a string representation of the injected proxy instance
        BundleContext context = FrameworkUtil.getBundle(getClass()).getBundleContext();

        ServiceReference nativeRef = ((ServiceReferenceProxy) ref).getTargetServiceReference();
        Object o = context.getService(nativeRef);

        try {
            IHello svcproxy = (IHello) o;
            ...
            context.ungetService(nativeRef);
        }
    }
     ...
     * 
     * Non-blocking remote service invocation storing the result inside a Future placeholder.
     * 
     * @param remotesvc
     */
     ...
     * 
     * With ECF >=3.3 remote services, when a proxy is created, if an interface class with the name [fq service
     * interface name]Async can be loaded and it extends IAsyncRemoteServiceProxy then the proxy will implement that
     * interface.
     * 
     * @param svcproxy
     */
